...
cc.Class({
        ...
        cc.resources.load("dialogs/dialogs", function (error, asset) {
            if (error) {
                ...
                self.dialogs = asset.json;//dialogs是数组，里面存着对话对象
                    ...
                    // }
                    // console.log("J="+j);
                    console.log(aweek.weekplan);
                    self.dialog = DialogParser.parseDialog(self.dialogs[aweek.weekplan[weeki]]);//dialog是混合类，结构在DialogDate中，分为属性和方法aweek.weekplan[weekid]
    ...
    stepDialog() {
        ...
        if(curr == -1)//如果当前为最后一句
        {
           ...
           weeki++;
            if(weeki < aweek.weekplan.length)
            {
                console.log("a dialog is end ~~~~~~~~~~~");
            this.node.off('touchend',this.stepDialog, this);
            this.optionPanel.removeAllChildren();
            ...
            button2.node.on('click', function(button){
                ...
                this.dialog = DialogParser.parseDialog(this.dialogs[aweek.weekplan[weeki]]);//dialog是混合类，结构在DialogDate中，分为属性和方法
                    // console.log("sequence "+self.sequence["Fine"]);
